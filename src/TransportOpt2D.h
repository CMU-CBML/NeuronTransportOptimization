...
private:
	...
	vector<double> Vel_plus, Vel_minus;

	vector<double> f_plus, f_minus;

	vector<double> lambda1, lambda2, lambda3, lambda4, lambda5; 
...
private:
	...
	void BasisFunction(double u, double v, int nen, const vector<array<double, 3>>& pt, const vector<array<double, 64>> &cmat, vector<double> &Nx, vector<array<double, 3>> &dNdx, vector<array<array<double, 3>, 3>> &dN2dx2, double dudx[3][3], double& detJ);
