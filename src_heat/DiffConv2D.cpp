...
void DiffConv2D::InitializeProblem(const UserSetting2D *ctx)
{
    ...
    ierr = MatSetSizes(Asubmat[0], PETSC_DECIDE, PETSC_DECIDE, state_num * nPoint * nTstep, state_num * nPoint * nTstep);
    ...
    ierr = MatSetSizes(Asubmat[1], PETSC_DECIDE, PETSC_DECIDE, state_num * nPoint * nTstep, ctrl_num * nPoint * nTstep);
    ...
    ierr = MatSetSizes(Asubmat[2], PETSC_DECIDE, PETSC_DECIDE, state_num * nPoint * nTstep, state_num * nPoint * nTstep);
    ...
    ierr = MatSetSizes(Asubmat[3], PETSC_DECIDE, PETSC_DECIDE, ctrl_num * nPoint * nTstep, state_num * nPoint * nTstep);
    ...
    ierr = MatSetSizes(Asubmat[4], PETSC_DECIDE, PETSC_DECIDE, ctrl_num * nPoint * nTstep, ctrl_num * nPoint * nTstep);
    ...
    ierr = MatSetSizes(Asubmat[5], PETSC_DECIDE, PETSC_DECIDE, ctrl_num * nPoint * nTstep, state_num * nPoint * nTstep);
    ...
    ierr = MatSetSizes(Asubmat[6], PETSC_DECIDE, PETSC_DECIDE, state_num * nPoint * nTstep, state_num * nPoint * nTstep);
    ...
    ierr = MatSetSizes(Asubmat[7], PETSC_DECIDE, PETSC_DECIDE, state_num * nPoint * nTstep, ctrl_num * nPoint * nTstep);
    ...
    ierr = MatSetSizes(Asubmat[8], PETSC_DECIDE, PETSC_DECIDE, state_num * nPoint * nTstep, state_num * nPoint * nTstep);
    ...
    ierr = VecCreateMPI(PETSC_COMM_WORLD, PETSC_DECIDE, nPoint * state_num * nTstep, &Y_bc);
    ...
    ierr = VecCreateMPI(PETSC_COMM_WORLD, PETSC_DECIDE, nPoint * state_num * nTstep, &Res_Desire);
...
void DiffConv2D::BasisFunction(double u, double v, int nen, const vector<array<double, dim>> &pt, const vector<array<double, bzpt_num>> &cmat, vector<double> &Nx, vector<array<double, dim>> &dNdx, vector<array<array<double, dim>, dim>> &dN2dx2, double dudx[dim][dim], double &detJ)
{
...
void DiffConv2D::BasisFunctionCoarseSpace(double u, double v, int nen, const vector<array<double, dim>> &pt, const vector<array<double, bzpt_num>> &cmat, vector<double> &Nx, vector<array<double, dim>> &dNdx, double &detJ)
{
    double Nu[2] = {(1. - u), u};
    double Nv[2] = {(1. - v), v};
...
void DiffConv2D::PointFormValue(vector<double> &Nx, const vector<double> &U, double Value)
{
    Value = 0.;
...
void DiffConv2D::PointFormGrad(vector<array<double, dim>> &dNdx, const vector<double> &U, double Value[dim])
{
    for (int j = 0; j < dim; j++)
        Value[j] = 0.;
...
void DiffConv2D::PointFormHess(vector<array<array<double, dim>, dim>> &d2Ndx2, const vector<double> &U, double Value[dim][dim])
{
...
void DiffConv2D::ResnlAssembly(vector<double> Eqtmp, const vector<int> &IEN, Vec &Gqtmp)
{
...
void DiffConv2D::StableMatAssembly(vector<vector<double>> Emat, const vector<int> &IEN, Mat &Gmat)
{
    int i, j, A, B, m, n;
    int nen = IEN.size();
...
void DiffConv2D::MatrixAssembly(vector<vector<double>> Emat, const vector<int> &IEN, Mat &Gmat)
{
    int i, j, A, B, m, n;
    int row_start, row_end, row_now;
    int add = 0;
...
void DiffConv2D::ComputeMassMatrix(vector<double> &Nx, const double detJ, vector<vector<double>> &MassMat)
{
    int a, b, nen = Nx.size();
    for (a = 0; a < nen; a++)
        for (b = 0; b < nen; b++)
...
void DiffConv2D::ComputeStiffMatrix(vector<array<double, dim>> &dNdx, const double detJ, vector<vector<double>> &StiffMat)
{
...
void DiffConv2D::ComputeConvectMatrix(vector<double> &v, vector<double> &Nx, vector<array<double, dim>> &dNdx, const double detJ, vector<vector<double>> &ConvectMat)
{
    int a, b, nen = Nx.size();
    double tmp = 0;
    for (a = 0; a < nen; a++)
...
void DiffConv2D::ComputeParMatrix(vector<double> &Nx, vector<array<double, dim>> &dNdx, const double detJ, int dir, vector<vector<double>> &ParMat)
{
    int a, b, nen = Nx.size();
    for (a = 0; a < nen; a++)
        for (b = 0; b < nen; b++)
...
void DiffConv2D::LocalL2Projection(int e, vector<double> w, vector<vector<double>> &u_proj)
{
...
void DiffConv2D::ComputeStableMatrix(const vector<double> val_ini[state_num], double h, vector<double> wdNdx_proj, vector<double> &Nx, vector<array<double, dim>> &dNdx, vector<array<array<double, dim>, dim>> &dN2dx2, const double detJ, const vector<int> &IEN, vector<vector<double>> &StableMat)
{
    int a, b, nen = IEN.size();
    double Pe, delta;
...
void DiffConv2D::ComputeStableMatrix_Convection(const vector<double> w, double h, vector<double> wdNdx_proj, vector<double> &Nx, vector<array<double, dim>> &dNdx, vector<array<array<double, dim>, dim>> &dN2dx2, const double detJ, const vector<int> &IEN, vector<vector<double>> &StableMat)
{
    int a, b, nen = IEN.size();
    double Pe, delta;
    double scale;
...
void DiffConv2D::ComputeResVector(const vector<int> bc_flag, const vector<double> w, const vector<double> val_ini[state_num], vector<double> &Nx, vector<array<double, dim>> &dNdx, const vector<int> &IEN, const double detJ, vector<double> &qtmp)
{
    int a, b, nen = IEN.size();
    // PetscInt *rows;
    // PetscReal *vals;
            ...
            if (bc_flag[IEN[i]] == -1)
            {
                for (int k = 0; k < nen; k++)
                {
                    if (k != i)
                    {
                        ...
                        qtmp[k + nen * 0 + j * nen * state_num] -= State[0][IEN[i]] * (par[0] * (dNdx[k][0] * dNdx[i][0] + dNdx[k][1] * dNdx[i][1]) + (w[0] * dNdx[k][0] + w[1] * dNdx[k][1])) * detJ;
                    }
                }
            }
        }
...
void DiffConv2D::GetMatrixPosition(int row, int n_var, int &i_point, int &i_var, int &i_tstep)
{
...
void DiffConv2D::BuildLinearSystemProcess(const vector<Vertex2D> &cpts, const vector<double> val_bc[state_num], const vector<double> val_ini[state_num])
{
...
void DiffConv2D::BuildLinearSystemProcessAsubmat(const vector<Vertex2D> &cpts, const vector<double> val_bc[state_num], const vector<double> val_ini[state_num])
{
    //Build linear system in each process
    int e;
    cout << "Process:" << comRank << " out of " << nProcess << " Start Loop for " << bzmesh_process.size() << " elements.\n";
    for (e = 0; e < bzmesh_process.size(); e++)
    {
        ...
        //Start element matrix assembly
        ElementFormMatrixA11(bzmesh_process[e].IEN, Mtmp, Ktmp, Convtmp, Pxtmp, Pytmp, Stabletmp);
        ElementFormMatrixA22(bzmesh_process[e].IEN, Mtmp, Ktmp, Convtmp, Pxtmp, Pytmp, Stabletmp);
        ElementFormMatrixA23(bzmesh_process[e].IEN, Mtmp, Ktmp, Convtmp, Pxtmp, Pytmp, Stabletmp);
        ElementFormMatrixA31(bzmesh_process[e].IEN, Mtmp, Ktmp, Convtmp, Pxtmp, Pytmp, Stabletmp);
...
void DiffConv2D::BuildResVectorProcess(const vector<Vertex2D> &cpts, const vector<double> val_bc[state_num], const vector<double> val_ini[state_num], const vector<int> bc_flag)
{
    //Build linear system in each process
    int e;
    ...
    for (e = 0; e < bzmesh_process.size(); e++)
    {
        ...
        for (int i = 0; i < Gpt.size(); i++)
        {
            for (int j = 0; j < Gpt.size(); j++)
            {
                BasisFunction(Gpt[i], Gpt[j], nen, bzmesh_process[e].pts, bzmesh_process[e].cmat, Nx, dNdx, dN2dx2, dudx, detJ);
                detJ = wght[i] * wght[j] * detJ;
                ComputeResVector(bc_flag, w, val_ini, Nx, dNdx, bzmesh_process[e].IEN, detJ, qtmp);
            }
        }
...
void DiffConv2D::ElementFormMatrixA11(const vector<int> &IEN, vector<vector<double>> EM, vector<vector<double>> EK, vector<vector<double>> EConv, vector<vector<double>> EPx, vector<vector<double>> EPy, vector<vector<double>> EStable)
{
    int i, j, A, B, m, n;
    for (int idx_time = 0; idx_time < nTstep;idx_time++)
    {
        for(int idx_var = 0; idx_var < state_num; idx_var++)
        {
            ...
            case 0:
                ...
                {
                    A = IEN[m];
                    rowList[m] = A + idx_var * nPoint + idx_time * nPoint * state_num;
                    for (n = 0; n < IEN.size(); n++)
                    {
                        B = IEN[n];
            ...
            MatSetValues(Asubmat[0], IEN.size(), rowList, IEN.size(), colList, tmpGmat, ADD_VALUES);
        }
    }    
}

void DiffConv2D::ElementFormMatrixA12(const vector<int> &IEN, vector<vector<double>> EM, vector<vector<double>> EK, vector<vector<double>> EConv, vector<vector<double>> EPx, vector<vector<double>> EPy, vector<vector<double>> EStable)
{
}

void DiffConv2D::ElementFormMatrixA13(const vector<int> &IEN, vector<vector<double>> EM, vector<vector<double>> EK, vector<vector<double>> EConv, vector<vector<double>> EPx, vector<vector<double>> EPy, vector<vector<double>> EStable)
{

}
void DiffConv2D::ElementFormMatrixA21(const vector<int> &IEN, vector<vector<double>> EM, vector<vector<double>> EK, vector<vector<double>> EConv, vector<vector<double>> EPx, vector<vector<double>> EPy, vector<vector<double>> EStable)
{

}
void DiffConv2D::ElementFormMatrixA22(const vector<int> &IEN, vector<vector<double>> EM, vector<vector<double>> EK, vector<vector<double>> EConv, vector<vector<double>> EPx, vector<vector<double>> EPy, vector<vector<double>> EStable)
{
    int i, j, A, B, m, n;
    for (int idx_time = 0; idx_time < nTstep; idx_time++)
    {
        for (int idx_var = 0; idx_var < ctrl_num; idx_var++)
        {
            ...
            case 0:
                ...
                {
                    A = IEN[m];
                    rowList[m] = A + idx_var * nPoint + idx_time * nPoint * ctrl_num;
                    for (n = 0; n < IEN.size(); n++)
                    {
                        B = IEN[n];
                        colList[n] = B + idx_var * nPoint + idx_time * nPoint * ctrl_num;
                        tmpGmat[add] = scale * EM[m][n];
                        add++;
                    }
                }
            ...
            MatSetValues(Asubmat[4], IEN.size(), rowList, IEN.size(), colList, tmpGmat, ADD_VALUES);
        }
    }
}
void DiffConv2D::ElementFormMatrixA23(const vector<int> &IEN, vector<vector<double>> EM, vector<vector<double>> EK, vector<vector<double>> EConv, vector<vector<double>> EPx, vector<vector<double>> EPy, vector<vector<double>> EStable)
{
    int i, j, A, B, m, n;
    for (int idx_time = 0; idx_time < nTstep; idx_time++)
    {
        for (int idx_var = 0; idx_var < ctrl_num; idx_var++)
        {
            ...
            MatSetValues(Asubmat[5], IEN.size(), rowList, IEN.size(), colList, tmpGmat, ADD_VALUES);
        }
    }
}
void DiffConv2D::ElementFormMatrixA31(const vector<int> &IEN, vector<vector<double>> EM, vector<vector<double>> EK, vector<vector<double>> EConv, vector<vector<double>> EPx, vector<vector<double>> EPy, vector<vector<double>> EStable)
{
    int i, j, A, B, m, n;
    for (int idx_time = 0; idx_time < nTstep; idx_time++)
    {
                ...
                if (idx_time > 0)
                {
                    ...
                    case 0:
                        ...
                        for (m = 0; m < IEN.size(); m++)
                        {
                            ...
                            {
                                B = IEN[n];
                                colList[2 * n + 0] = B + idx_var * nPoint + (idx_time - 1) * nPoint * state_num;
                                colList[2 * n + 1] = B + idx_var * nPoint + idx_time * nPoint * state_num;
                                tmpGmat[add] = -EM[m][n];
                                tmpGmat[add + 1] = EM[m][n] + dt * par[0] * EK[m][n] + EStable[m + idx_time * IEN.size()][n];
                                add += 2;
                            }
                        }
                        break;
                    ...
                    MatSetValues(Asubmat[6], IEN.size(), rowList, IEN.size() * 2, colList, tmpGmat, ADD_VALUES);
                }
                else
                {
                    add = 0;
                    ...
                    case 0:
                        for (m = 0; m < IEN.size(); m++)
                        {
                            ...
                            for (n = 0; n < IEN.size(); n++)
                            {
                                B = IEN[n];
                                colList[n] = B + idx_var * nPoint + idx_time * nPoint * state_num;
                                tmpGmat[add] = EM[m][n] + dt * par[0] * EK[m][n] + EStable[m + idx_time * IEN.size()][n];
                                add++;
                            }
                        }
                        break;
                    ...
                    MatSetValues(Asubmat[6], IEN.size(), rowList, IEN.size(), colList, tmpGmat, ADD_VALUES);
                }
            }
            else
            {
                ...
                for (m = 0; m < IEN.size(); m++)
                {
                    ...
                    for (n = 0; n < IEN.size(); n++)
                    {
                        ...
                        // ! Convection
                        tmpGmat[add] = par[0] * EK[m][n] + EConv[m][n] + EStable[m + idx_time * IEN.size()][n];
...
void DiffConv2D::ElementFormMatrixA32(const vector<int> &IEN, vector<vector<double>> EM, vector<vector<double>> EK, vector<vector<double>> EConv, vector<vector<double>> EPx, vector<vector<double>> EPy, vector<vector<double>> EStable)
{

}
void DiffConv2D::ElementFormMatrixA33(const vector<int> &IEN, vector<vector<double>> EM, vector<vector<double>> EK, vector<vector<double>> EConv, vector<vector<double>> EPx, vector<vector<double>> EPy, vector<vector<double>> EStable)
{
...
void DiffConv2D::AssembleGlobalMatrix(Mat &A)
{
    PetscInt row, start, end;

    MatCreate(PETSC_COMM_WORLD, &A);
    MatSetSizes(A, PETSC_DECIDE, PETSC_DECIDE, (2 * state_num + ctrl_num) * nPoint * nTstep, (2 * state_num + ctrl_num) * nPoint * nTstep);
    MatSetType(A, MATMPIAIJ);
    MatMPIAIJSetPreallocation(A, nPoint * (state_num + ctrl_num), NULL, nPoint * (state_num + ctrl_num), NULL);
    // MatSetOption(A,MAT_NEW_NONZERO_ALLOCATION_ERR,PETSC_FALSE);
    MatSetUp(A);
    PetscObjectSetName((PetscObject)A, "TanMatAIJ");
    MatGetOwnershipRange(A, &start, &end);

    PetscInt shift[3] = {0, state_num * nTstep * nPoint, (state_num + ctrl_num) * nTstep * nPoint};
    PetscInt size[3] = {state_num * nTstep * nPoint,  ctrl_num * nTstep * nPoint, state_num * nTstep * nPoint};

    for (row = start; row < end; row++)
    {
        PetscInt ncols_1, ncols_2, ncols_3;
        const PetscReal *vals_1, *vals_2, *vals_3;
        const PetscInt *cols_1, *cols_2, *cols_3;

        PetscInt *col;
        PetscReal *vals;
        PetscReal scale;

        IS is_row1, is_row2, is_row3;
        IS is_col1, is_col2, is_col3;
        IS *is_row_array1, *is_col_array1;
        IS *is_row_array2, *is_col_array2;
        IS *is_row_array3, *is_col_array3;
        Mat *Asub1, *Asub2, *Asub3;

        PetscMalloc1(1, &is_row_array1);
        PetscMalloc1(1, &is_col_array1);
        PetscMalloc1(1, &is_row_array2);
        PetscMalloc1(1, &is_col_array2);
        PetscMalloc1(1, &is_row_array3);
        PetscMalloc1(1, &is_col_array3);
        PetscMalloc1(1, &Asub1);
        PetscMalloc1(1, &Asub2);
        PetscMalloc1(1, &Asub3);

        if (row < shift[1])
        {
            ISCreateStride(PETSC_COMM_SELF, 1, row - shift[0], 1, &is_row1);
            ISCreateStride(PETSC_COMM_SELF, 1, row - shift[0], 1, &is_row2);
            ISCreateStride(PETSC_COMM_SELF, 1, row - shift[0], 1, &is_row3);
            ISCreateStride(PETSC_COMM_SELF, size[0], 0, 1, &is_col1);
            ISCreateStride(PETSC_COMM_SELF, size[1], 0, 1, &is_col2);
            ISCreateStride(PETSC_COMM_SELF, size[2], 0, 1, &is_col3);

            is_row_array1[0] = is_row1;
            is_col_array1[0] = is_col1;
            is_row_array2[0] = is_row2;
            is_col_array2[0] = is_col2;
            is_row_array3[0] = is_row3;
            is_col_array3[0] = is_col3;

            MatCreateSubMatrices(Asubmat[0], 1, is_row_array1, is_col_array1, MAT_INITIAL_MATRIX, &Asub1);
            MatCreateSubMatrices(Asubmat[1], 1, is_row_array2, is_col_array2, MAT_INITIAL_MATRIX, &Asub2);
            MatCreateSubMatrices(Asubmat[2], 1, is_row_array3, is_col_array3, MAT_INITIAL_MATRIX, &Asub3);
        }
        else if (row < shift[2])
        {
            ISCreateStride(PETSC_COMM_SELF, 1, row - shift[1], 1, &is_row1);
            ISCreateStride(PETSC_COMM_SELF, 1, row - shift[1], 1, &is_row2);
            ISCreateStride(PETSC_COMM_SELF, 1, row - shift[1], 1, &is_row3);
            ISCreateStride(PETSC_COMM_SELF, size[0], 0, 1, &is_col1);
            ISCreateStride(PETSC_COMM_SELF, size[1], 0, 1, &is_col2);
            ISCreateStride(PETSC_COMM_SELF, size[2], 0, 1, &is_col3);
            ...
            MatCreateSubMatrices(Asubmat[3], 1, is_row_array1, is_col_array1, MAT_INITIAL_MATRIX, &Asub1);
            MatCreateSubMatrices(Asubmat[4], 1, is_row_array2, is_col_array2, MAT_INITIAL_MATRIX, &Asub2);
            MatCreateSubMatrices(Asubmat[5], 1, is_row_array3, is_col_array3, MAT_INITIAL_MATRIX, &Asub3);
        }
        else
        {
            ISCreateStride(PETSC_COMM_SELF, 1, row - shift[2], 1, &is_row1);
            ISCreateStride(PETSC_COMM_SELF, 1, row - shift[2], 1, &is_row2);
            ISCreateStride(PETSC_COMM_SELF, 1, row - shift[2], 1, &is_row3);
            ISCreateStride(PETSC_COMM_SELF, size[0], 0, 1, &is_col1);
            ISCreateStride(PETSC_COMM_SELF, size[1], 0, 1, &is_col2);
            ISCreateStride(PETSC_COMM_SELF, size[2], 0, 1, &is_col3);
            ...
            MatCreateSubMatrices(Asubmat[6], 1, is_row_array1, is_col_array1, MAT_INITIAL_MATRIX, &Asub1);
            MatCreateSubMatrices(Asubmat[7], 1, is_row_array2, is_col_array2, MAT_INITIAL_MATRIX, &Asub2);
            MatCreateSubMatrices(Asubmat[8], 1, is_row_array3, is_col_array3, MAT_INITIAL_MATRIX, &Asub3);
        }
        

        MatGetRow(Asub1[0], 0, &ncols_1, &cols_1, &vals_1);
        MatGetRow(Asub2[0], 0, &ncols_2, &cols_2, &vals_2);
        MatGetRow(Asub3[0], 0, &ncols_3, &cols_3, &vals_3);

        // cout << "Ncols_1: " << ncols_1 <<endl;
        // for(int i =0;i<ncols_1;i++)
        // 	cout << "Col1: "<< cols_1[i] << " Value: " << vals_1[i] <<endl;
        // cout << "Ncols_2: " << ncols_2 <<endl;
        // for(int i =0;i<ncols_2;i++)
        // 	cout << "Col2: "<< cols_2[i] << " Value: " << vals_2[i] <<endl;
        // cout << "Ncols_3: " << ncols_3 << endl;
        // for (int i = 0; i < ncols_3; i++)
        //     cout << "Col3: " << cols_3[i] << " Value: " << vals_3[i] << endl;

        PetscMalloc1(ncols_1 + ncols_2 + ncols_3, &vals);
        PetscMalloc1(ncols_1 + ncols_2 + ncols_3, &col);

        int count = 0;
        for (int i = 0; i < ncols_1; i++)
        {
            vals[count] = vals_1[i];
            col[count] = cols_1[i] + shift[0];
            count++;
        }
        ...
        {
            vals[count] = vals_3[i];
            col[count] = cols_3[i] + shift[2];
            count++;
        }

        MatSetValues(A, 1, &row, count, col, vals, INSERT_VALUES);

        MatRestoreRow(Asub1[0], 0, &ncols_1, &cols_1, &vals_1);
        MatRestoreRow(Asub2[0], 0, &ncols_2, &cols_2, &vals_2);
        MatRestoreRow(Asub3[0], 0, &ncols_3, &cols_3, &vals_3);
        ...
        // ISDestroy(&is_col2);
        // ISDestroy(&is_col3);
        // ISDestroy(&is_row_array1[0]);
        // ISDestroy(&is_row_array2[0]);
        // ISDestroy(&is_row_array3[0]);
        // ISDestroy(&is_col_array1[0]);
        // ISDestroy(&is_col_array2[0]);
        // ISDestroy(&is_col_array3[0]);
        // MatDestroy(&Asub1[0]);
        // MatDestroy(&Asub2[0]);
        // MatDestroy(&Asub3[0]);
        ...
        // PetscFree(col);
        // PetscFree(vals);
        // PetscFree(is_row_array1);
        // PetscFree(is_row_array2);
        // PetscFree(is_row_array3);
        // PetscFree(is_col_array1);
        // PetscFree(is_col_array2);
        // PetscFree(is_col_array3);
        // PetscFree(Asub1);
        // PetscFree(Asub2);
        // PetscFree(Asub3);
    }

    PetscPrintf(PETSC_COMM_WORLD, "Start Assemble Global Matrix...\n");
    MatAssemblyBegin(A, MAT_FINAL_ASSEMBLY);
    MatAssemblyEnd(A, MAT_FINAL_ASSEMBLY);
}
...
void DiffConv2D::ApplyBoundaryCondition(const UserSetting2D *ctx, int flag)
{
    ...
    // Vec Rhs_BC;
    // VecCreateMPI(PETSC_COMM_WORLD, PETSC_DECIDE, (state_num * 2 + ctrl_num) * nPoint * nTstep, &Rhs_BC);
    // VecSet(Rhs_BC, 0.0);
    ...
    VecSetValues(ResVec, nTstep * n_bcval * 2, bc_global_ids, bc_vals, INSERT_VALUES); // ! Modify to test boundary condition
                ...
                for (int tt = 0; tt < nTstep; tt++)
                {
                    ...
                    for (int k = 0; k < nen; k++)
                    {
                        ...
                        for (int s = 0; s < state_num; s++)
                            qtmp[tt * state_num * nen + s * nen + k] += tmp_desire[s] * Nx[k] * detJ;
                    }
                }
            }
        }
...
void DiffConv2D::ResidualVecSetup(Vec x)
{
    ...
    for (k = 0; k < 3; k++)
    {
        ...
        // cout << "Rank: " << comRank <<" k: " << k << " low " << low << " high " << high << "\n";
        for (i = 0; i < ldim; i++)
        {
            val = array[i];
...
// ? Assemble Submatrix directly, needs debugging
    ...
    for (l = 0; l < n_iterator; l++)
    {
        ...
        t0 = time(NULL);
        // TangentMatSetup();
        TangentMatSetupAsubmat(); // ? Assemble Submatrix directly, needs debugging
        t1 = time(NULL);
        PetscPrintf(PETSC_COMM_WORLD, "Done Matrix Assembly with time: %d \n", t1 - t0);
...
void DiffConv2D::VisualizeVTK_ControlMesh_Heat(const vector<Vertex2D> &spt, const vector<Element2D> &mesh, int time, int step, string fn)
{
    stringstream ss;
    ss << step << "_" << time;
...
void DiffConv2D::VisualizeVTK_ControlMesh_Burger(const vector<Vertex2D> &spt, const vector<Element2D> &mesh, int time, int step, string fn)
{
    stringstream ss;
    ss << step << "_" << time;
...
void DiffConv2D::VisualizeVTK_ControlMesh(const vector<Vertex2D> &spt, const vector<Element2D> &mesh, int step, string fn)
{
    stringstream ss;
    ss << step;
    ...
    for (unsigned int e = 0; e < num_bzmesh_ele; e++)
    {
        ...
        for (int a = 0; a < ns; a++)
        {
            for (int b = 0; b < ns; b++)
            {
                ...
                // cout << " spt: " << loc << " " << pt1[0] << " " << pt1[1] << " " << pt1[2] << "\n";
...
void DiffConv2D::WriteVTK(const vector<array<double, 3>> spt, const vector<double> sdisp, const vector<array<int, 4>> sele, int time, int step, string fn)
{
    stringstream ss;
    ss << step << "_" << time;
...
void DiffConv2D::ResultCal_Bezier(double u, double v, int time, const Element2D &bzel, double pt[2], double result[result_num], double dudx[3], double &detJ)
{
    double dUdx[dim][dim];
...
// For 3D
// void DiffConv2D::BasisFunction(double u, double v, double w, int nen, const vector<array<double, 3>>& pt, const vector<array<double, 64>> &cmat, vector<double> &Nx, vector<array<double, 3>> &dNdx, vector<array<array<double, 3>, 3>> &dN2dx2, double dudx[3][3], double& detJ)
// {
// 	double Nu[4] = { (1. - u)*(1. - u)*(1. - u), 3.*(1. - u)*(1. - u)*u, 3.*(1. - u)*u*u, u*u*u };
// 	double Nv[4] = { (1. - v)*(1. - v)*(1. - v), 3.*(1. - v)*(1. - v)*v, 3.*(1. - v)*v*v, v*v*v };
...
// void DiffConv2D::WriteVTK(const vector<array<double, 3>> spt, const vector<double> sdisp, const vector<array<int,8>> sele, int step, string fn)
// {
...
// void DiffConv2D::ResultCal_Bezier(double u, double v, const Element2D& bzel, double pt[3], double result[4], double dudx[3], double& detJ)
// {
// 	double dUdx[dim][dim];
// 	vector<double> Nx(bzel.IEN.size());
// 	vector<array<double, dim>> dNdx(bzel.IEN.size());
