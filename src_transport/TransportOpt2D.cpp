...
void TransportOpt2D::FormMatrixA23(Mat M, Mat K, Mat &A)
{
	...
	for(row = start; row < end; row++)
	{
		...
		// ? Pay attention to the "ind_var + 3" here
		for(int i = 0; i < ncols_m; i++)
		{
			vals[i] = vals_m[i]*scale;
		...
		if(time_int != 0)
		{
			if (ind_time > 0)
			{
				...
				case 0:
					...
					{
						vals[i + 0 * ncols_m] = -vals_m[i];
						vals[i + 1 * ncols_m] = vals_m[i] * (1 + dt * par[5] + dt * par[6]);
						vals[i + 2 * ncols_m] = vals_m[i] * (-dt * par[7]);
						vals[i + 3 * ncols_m] = vals_m[i] * (-dt * par[8]);
						col[i + 0 * ncols_m] = cols_m[i] + 0 * nPoint + (ind_time - 1) * nPoint * state_num;
						col[i + 1 * ncols_m] = cols_m[i] + 0 * nPoint + ind_time * nPoint * state_num;
						col[i + 2 * ncols_m] = cols_m[i] + 1 * nPoint + ind_time * nPoint * state_num;
						col[i + 3 * ncols_m] = cols_m[i] + 2 * nPoint + ind_time * nPoint * state_num;
				...
				case 1:
					...
					for (int i = 0; i < ncols_m; i++)
					{
						vals[i + 0 * ncols_m] = -vals_m[i];
						vals[i + 1 * ncols_m] = vals_m[i] * (-dt * par[5]);
						vals[i + 2 * ncols_m] = vals_m[i] * (1 + dt * par[7]) + vals_k[i] * (dt * par[1]);
						col[i + 0 * ncols_m] = cols_m[i] + 1 * nPoint + (ind_time - 1) * nPoint * state_num;
						col[i + 1 * ncols_m] = cols_m[i] + 0 * nPoint + ind_time * nPoint * state_num;
						col[i + 2 * ncols_m] = cols_m[i] + 1 * nPoint + ind_time * nPoint * state_num;
					}
				...
				case 2:
					...
					for (int i = 0; i < ncols_m; i++)
					{
						vals[i + 0 * ncols_m] = -vals_m[i];
						vals[i + 1 * ncols_m] = vals_m[i] * (-dt * par[6]);
						vals[i + 2 * ncols_m] = vals_m[i] * (1 + dt * par[8]) + vals_k[i] * (dt * par[2]);
						col[i + 0 * ncols_m] = cols_m[i] + 2 * nPoint + (ind_time - 1) * nPoint * state_num;
						col[i + 1 * ncols_m] = cols_m[i] + 0 * nPoint + ind_time * nPoint * state_num;
						col[i + 2 * ncols_m] = cols_m[i] + 2 * nPoint + ind_time * nPoint * state_num;
					}
				...
				case 3:
					...
					{
						vals[i + 0 * ncols_m] = -vals_m[i];
						vals[i + 1 * ncols_m] = vals_m[i] + dt * par[10] * vals_k[i];
						col[i + 0 * ncols_m] = cols_m[i] + ind_var * nPoint + (ind_time - 1) * nPoint * state_num;
						col[i + 1 * ncols_m] = cols_m[i] + ind_var * nPoint + ind_time * nPoint * state_num;
					}
					for (int i = 0; i < ncols_px; i++)
					{
						vals[i + 2 * ncols_m] = vals_px[i] * dt * par[9] * par[9];
				...
				case 4:
					...
					for (int i = 0; i < ncols_py; i++)
					{
						vals[i + 2 * ncols_m] = vals_py[i] * dt * par[9] * par[9];
			...
			else
			{
				...
				case 0:
					...
					for (int i = 0; i < ncols_m; i++)
					{
						vals[i + 0 * ncols_m] = vals_m[i] * (1 + dt * par[5] + dt * par[6]);
						vals[i + 1 * ncols_m] = vals_m[i] * (-dt * par[7]);
						vals[i + 2 * ncols_m] = vals_m[i] * (-dt * par[8]);
				...
				case 1:
					...
					for (int i = 0; i < ncols_m; i++)
					{
						vals[i + 0 * ncols_m] = vals_m[i] * (-dt * par[5]);
						vals[i + 1 * ncols_m] = vals_m[i] * (1 + dt * par[7]) + vals_k[i] * (dt * par[1]);
						col[i + 0 * ncols_m] = cols_m[i] + 0 * nPoint + ind_time * nPoint * state_num;
						col[i + 1 * ncols_m] = cols_m[i] + 1 * nPoint + ind_time * nPoint * state_num;
					}
					MatSetValues(A, 1, &row, (2 * ncols_m), col, vals, INSERT_VALUES);
				...
				case 2:
					...
					for (int i = 0; i < ncols_m; i++)
					{
						vals[i + 0 * ncols_m] = vals_m[i] * (-dt * par[6]);
						vals[i + 1 * ncols_m] = vals_m[i] * (1 + dt * par[8]) + vals_k[i] * (dt * par[2]);
						col[i + 0 * ncols_m] = cols_m[i] + 0 * nPoint + ind_time * nPoint * state_num;
				...
				case 3:
					...
					for (int i = 0; i < ncols_m; i++)
					{
						vals[i + 0 * ncols_m] = vals_m[i] + dt * par[10] * vals_k[i];
						col[i + 0 * ncols_m] = cols_m[i] + ind_var * nPoint + ind_time * nPoint * state_num;
					}
					for (int i = 0; i < ncols_px; i++)
					{
						vals[i + 1 * ncols_m] = vals_px[i] * dt * par[9] * par[9];
				...
				case 4:
					...
					for (int i = 0; i < ncols_py; i++)
					{
						vals[i + 1 * ncols_m] = vals_py[i] * dt * par[9] * par[9];
...
void TransportOpt2D::ApplyBoundaryCondition(const UserSetting2D *ctx, int flag)
{
	...
	for (int j = 0; j < nTstep; j++)
	{
		for (int i = 0; i < ctx->bc_flag.size(); i++)
		{
			...
			{
				bc_global_ids[count + 0] = i + 0 * nPoint + j * state_num * nPoint;
				bc_vals[count + 0] = ctx->val_bc[0][i] - ctx->val_bc[0][i];
				bc_global_ids[count + 1] = i + 1 * nPoint + j * state_num * nPoint;
				bc_vals[count + 1] = ctx->val_bc[1][i] - ctx->val_bc[1][i];
				bc_global_ids[count + 2] = i + 2 * nPoint + j * state_num * nPoint;
				bc_vals[count + 2] = ctx->val_bc[2][i] - ctx->val_bc[2][i];
				bc_global_ids[count + 3] = i + 3 * nPoint + j * state_num * nPoint;
				bc_vals[count + 3] = ctx->val_bc[3][i] - ctx->val_bc[3][i];
				bc_global_ids[count + 4] = i + 4 * nPoint + j * state_num * nPoint;
				bc_vals[count + 4] = ctx->val_bc[4][i] - ctx->val_bc[4][i];
				count += 5;
				continue;
			}
			else if (ctx->bc_flag[i] == -2)
			{
				...
				bc_global_ids[count + 2] = i + 2 * nPoint + j * state_num * nPoint;
				bc_vals[count + 2] = ctx->val_bc[2][i] - ctx->val_bc[2][i];
				bc_global_ids[count + 3] = i + 5 * nPoint + j * state_num * nPoint;
				bc_vals[count + 3] = ctx->val_bc[5][i] - ctx->val_bc[5][i];
				bc_global_ids[count + 4] = i + 6 * nPoint + j * state_num * nPoint;
				bc_vals[count + 4] = ctx->val_bc[6][i] - ctx->val_bc[6][i];
				count += 5;
				continue;
			}
			else if (ctx->bc_flag[i] == -3)
			{
				bc_global_ids[count + 0] = i + 3 * nPoint + j * state_num * nPoint;
				bc_vals[count + 0] = ctx->val_bc[3][i] - ctx->val_bc[3][i];
				bc_global_ids[count + 1] = i + 4 * nPoint + j * state_num * nPoint;
				bc_vals[count + 1] = ctx->val_bc[4][i] - ctx->val_bc[4][i];
				bc_global_ids[count + 2] = i + 5 * nPoint + j * state_num * nPoint;
				bc_vals[count + 2] = ctx->val_bc[5][i] - ctx->val_bc[5][i];
				bc_global_ids[count + 3] = i + 6 * nPoint + j * state_num * nPoint;
				bc_vals[count + 3] = ctx->val_bc[6][i] - ctx->val_bc[6][i];
				count += 4;
				continue;
			}
		}
...
void TransportOpt2D::FormResVecb2(Vec x)
{
	...
	// cout << "Size of x: " << size << "\n";
	// VecGetSize(y_k, &size);
	// cout << "Size of y_k: " << size << "\n";
	// VecGetSize(u_k, &size);
	// cout << "Size of u_k: " << size << "\n";
	// VecGetSize(l_k, &size);
	// cout << "Size of l_k: " << size << "\n";
	// DebugVisualizeVec(y_k, work_dir + "debug/y_k.m");
	// DebugVisualizeVec(u_k, work_dir + "debug/u_k.m");
	// DebugVisualizeVec(l_k, work_dir + "debug/l_k.m");
...
void TransportOpt2D::VisualizeVTK_ControlMesh_Transport(const vector<Vertex2D> &spt, const vector<Element2D> &mesh, int time, int step, string fn)
{
	unsigned int i;
	stringstream ss;
	ss << step << "_" << time;
	...
	if (fout.is_open())
	{
		...
		for (i = 0; i < spt.size(); i++)
			fout << std::setprecision(9) << State[1][i + time * nPoint] << "\n";
		fout << "SCALARS nminus float 1\nLOOKUP_TABLE default\n";
		for (i = 0; i < spt.size(); i++)
			fout << std::setprecision(9) << State[2][i + time * nPoint] << "\n";
		fout << "VECTORS vplus float\n";
		for (i = 0; i < spt.size(); i++)
			fout << std::setprecision(9) << State[3][i + time * nPoint] << " " << std::setprecision(9) << State[4][i + time * nPoint] << " " << 0 << "\n";
		fout << "VECTORS vminus float\n";
		for (i = 0; i < spt.size(); i++)
			fout << std::setprecision(9) << State[5][i + time * nPoint] << " " << std::setprecision(9) << State[6][i + time * nPoint] << " " << 0 << "\n";
		...
		for (i = 0; i < spt.size(); i++)
			fout << std::setprecision(9) << Ctrl[2][i + time * nPoint] << " " << std::setprecision(9) << Ctrl[3][i + time * nPoint] << " " << 0 << "\n";
		// ! Output penalty variable
		fout << "SCALARS p1 float 1\nLOOKUP_TABLE default\n";
		for (i = 0; i < spt.size(); i++)
			fout << std::setprecision(9) << Lambda[0][i + time * nPoint] << "\n";
		fout << "SCALARS p2 float 1\nLOOKUP_TABLE default\n";
		for (i = 0; i < spt.size(); i++)
			fout << std::setprecision(9) << Lambda[1][i + time * nPoint] << "\n";
		fout << "SCALARS p3 float 1\nLOOKUP_TABLE default\n";
		for (i = 0; i < spt.size(); i++)
			fout << std::setprecision(9) << Lambda[2][i + time * nPoint] << "\n";
		fout << "SCALARS p4 float 1\nLOOKUP_TABLE default\n";
		for (i = 0; i < spt.size(); i++)
			fout << std::setprecision(9) << Lambda[3][i + time * nPoint] << "\n";
		fout << "SCALARS p5 float 1\nLOOKUP_TABLE default\n";
		for (i = 0; i < spt.size(); i++)
			fout << std::setprecision(9) << Lambda[4][i + time * nPoint] << "\n";
		fout << "SCALARS p6 float 1\nLOOKUP_TABLE default\n";
		for (i = 0; i < spt.size(); i++)
			fout << std::setprecision(9) << Lambda[5][i + time * nPoint] << "\n";
		fout << "SCALARS p7 float 1\nLOOKUP_TABLE default\n";
		for (i = 0; i < spt.size(); i++)
			fout << std::setprecision(9) << Lambda[6][i + time * nPoint] << "\n";
		fout.close();
	}
	else
	{
...
void TransportOpt2D::VisualizeVTK_PhysicalDomain(int time, int step, string fn)
{
	...
	// stringstream ss;
	// ss << comRank;
	// string fname = work_dir + "/debug/CM_vis" + ss.str() + ".txt";
	// ofstream fout;
	// fout.open(fname.c_str());
	// if (fout.is_open())
	// {
	// 	for (int i = 0; i < num_bzmesh_ele;i++)
	// 	{
	// 		for (int j = 0; j < ns_pt;j++)
	// 		{
	// 			fout << spt_proc[i * ns_pt * 3 + j * 3 + 0] << " " << spt_proc[i * ns_pt * 3 + j * 3 + 1] << " " << spt_proc[i * ns_pt * 3 + j * 3 + 2] << "\n";
	// 		}
	// 	}
	// 	fout.close();
	// }
	...
	// if (comRank == 0)
	// {
	// 	string fname1 = work_dir + "/debug/spts_all.txt";
	// 	ofstream fout1;
	// 	fout1.open(fname1.c_str());
	// 	if (fout1.is_open())
	// 	{
	...
	// 			for (int j = 0; j < ns_pt; j++)
	// 			{
	// 				fout1 << spts[i * ns_pt * 3 + j * 3 + 0] << " " << spts[i * ns_pt * 3 + j * 3 + 1] << " " << spts[i * ns_pt * 3 + j * 3 + 2] << "\n";
	// 			}
	// 		}
	// 		fout1.close();
	// 	}
