function [] = GenHex_ExtractQuad(io_path)
    %% User settings
    ...
    quad_path = [io_path, 'quad//'];
    hex_path = [io_path, 'hex//'];
    
    if ~exist(quad_path, 'dir')
        mkdir(quad_path)
    end
    if ~exist(hex_path, 'dir')
        mkdir(hex_path)
    end
    
    velocity_output = [hex_path, 'initial_velocityfield.txt'];
    
    hex_output = [hex_path, 'controlmesh.vtk'];
    hex_info_output = [hex_path, 'controlmesh_info.txt'];
    hex_extract_output = [hex_path, '//mesh_extract.vtk'];
    quad_extract_output = [io_path, 'controlmesh.vtk'];
    mapping_extract_output = [hex_path, '//C2Dmapping_extract.txt'];
    ...
    extract_p = load('.//template//extraction_quad//template_circle_point90_extract_index.txt');
    extract_merge_p = load('.//template//extraction_quad//template_merge120_points90_extract_index.txt');

%     extract_p_theta = load('.//template//extraction_quad//template_circle_point90_extract_index_theta.txt');

    extract_p(:, 2) = extract_p(:, 2) + 1;
    extract_merge_p(:, 2) = extract_merge_p(:, 2) + 1;

    extract_e = load('.//template//extraction_quad//template_circle_elements_extract_index.txt');
    extract_merge_e = load('.//template//extraction_quad//template_merge_elements_extract_index.txt');

    extract_merge_e_bot = load('.//template//extraction_quad//template_merge_elements_extract_index_bot.txt');
    extract_merge_e_left = load('.//template//extraction_quad//template_merge_elements_extract_index_left.txt');
    extract_merge_e_right = load('.//template//extraction_quad//template_merge_elements_extract_index_right.txt');
    ...
    for index_bif = 1:n_bif
        ...
        for k = 1:m4_extract
            tmp_me_extract(k, 1:2) = extract_merge_e(k, 2:3) + tmp_pointnumber_extract * [1, 1];
        end

        for k = 1:m2_extract
            tmp_branch_be_extract(k, 1:2) = extract_merge_e_bot(k, 2:3) + tmp_pointnumber_extract * [1, 1];
            tmp_branch_le_extract(k, 1:2) = extract_merge_e_left(k, 2:3) + tmp_pointnumber_extract * [1, 1];
            tmp_branch_re_extract(k, 1:2) = extract_merge_e_right(k, 2:3) + tmp_pointnumber_extract * [1, 1];
        end

        % LayerElement_extract = [LayerElement_extract; tmp_me_extract(:, 1:2)];
        NodeLayer_extract{i, 1} = tmp_mp(tmp_extract_mp(:, 2));
        NodeLayer_extract{i, 2} = tmp_branch_be_extract(:, 1:2);
        NodeLayer_extract{i, 3} = tmp_branch_le_extract(:, 1:2);
        NodeLayer_extract{i, 4} = tmp_branch_re_extract(:, 1:2);
        % [idx_bif, ~] = find(dict_bif(:, 2) == i);
        ...
        for k = 1:m2_extract
            tmp_e_extract(k, 1:2) = extract_e(k, 2:3) + tmp_pointnumber_extract * [1, 1];
        end

        % LayerElement_extract = [LayerElement_extract; tmp_e_extract(:, 1:2)];
        NodeLayer_extract{par_node, 1} = tmp_bpt(tmp_extract_p(:, 2));
        NodeLayer_extract{par_node, 2} = tmp_e_extract(:, 1:2);
        NodeLayer_extract{par_node, 3} = tmp_e_extract(:, 1:2);
        ...
        NodeLayer_extract{bifur1_node, 2} = tmp_e_extract(:, 1:2);
        NodeLayer_extract{bifur1_node, 3} = tmp_e_extract(:, 1:2);
        ...
        % LayerElement_extract = [LayerElement_extract; tmp_e(:, 1:2)];
        NodeLayer_extract{bifur2_node, 1} = tmp_rpt(tmp_extract_p(:, 2));
        NodeLayer_extract{bifur2_node, 2} = tmp_e_extract(:, 1:2);
        NodeLayer_extract{bifur2_node, 3} = tmp_e_extract(:, 1:2);
        ...
        for index_sec = 1:n_sect
            ...
            if (index_start_pt == 1)
                ...
                for index_sec_pt = sect_ptnum:-1:2
                    ...
                    % LayerElement_extract = [LayerElement; tmp_e(:, 1:2)];
                    NodeLayer_extract{j, 1} = tmp_p(extract_p(:, 2));
                    NodeLayer_extract{j, 2} = tmp_e_extract(:, 1:2);
                    NodeLayer_extract{j, 3} = tmp_e_extract(:, 1:2);
            ...
            if (termination(index_end_pt))
                ...
                for index_sec_pt = 2:sect_ptnum
                    ...
                    % LayerElement_extract = [LayerElement; tmp_e_extract(:, 1:2)];
                    NodeLayer_extract{i, 1} = tmp_p(extract_p(:, 2));
                    NodeLayer_extract{i, 2} = tmp_e_extract(:, 1:2);
                    NodeLayer_extract{i, 3} = tmp_e_extract(:, 1:2);
                end
...
%             angle_total_extract = angle_total;
            ...
            % if angle_total_extract > pi / 4 && angle_total_extract <= 3 * pi / 4
            %     angle_total_extract = angle_total_extract - pi / 2;
            %     %             if dot(rotate_axis,sec_vec_end)>0
            %     if dot(rotate_axis, n_end) > 0
            %         %                     if dot(rotate_axis,[0 0 1])>0
            %         for index_ele = 1:m2_extract / 2
            %             tmp_end_element_extract(index_ele + m2_extract / 2 * 0, :) = NodeLayer_extract{index_end_pt, 2}(index_ele + m2_extract / 2 * 1, :);
            %             tmp_end_element_extract(index_ele + m2_extract / 2 * 1, :) = NodeLayer_extract{index_end_pt, 2}(index_ele + m2_extract / 2 * 0, :);
            %         end
            ...
            if angle_total_extract > pi /2 && angle_total_extract <= pi
                angle_total_extract = angle_total_extract - pi;
    ...
    for index_sec = 1:n_sect
        ...
        for index_sec_pt = 1:sect_ptnum - 1
            ...
            if segment_layer == 1
                ...
                % * Extraction
                [tmp_pointnumber_extract, ~] = size(AllPoint_extract);
                AllElement_extract = [AllElement_extract; tmp_start_e_extract, tmp_end_e_extract(:,[2,1])];
                ...
                for k = 1:segment_layer - 1
                    ...
                    for iele = 1:m2_extract
                        tmp_e_extract(iele, 1:2) = extract_e(iele, 2:3) + tmp_pointnumber_extract * [1, 1];
                    end

                    % LayerElement_extract = [LayerElement; tmp_e_extract(:, 1:2)];

                    if (k == 1)
                        AllElement_extract = [AllElement_extract; tmp_start_e_extract, tmp_e_extract(:, [2,1])];
                        AllElement_extract = [AllElement_extract; tmp_e_extract(:, 1:2), zeros(m2_extract, 2)];
                        % AllElementLabel_extract = [AllElementLabel_extract; ones(size(tmp_start_e_extract, 1), 1) * NodeLayer_extract{i, 5}(row)];
                        % AllElementLabel_extract = [AllElementLabel_extract; ones(size(tmp_e_extract, 1), 1) * NodeLayer_extract{i, 5}(row)];

                    else
                        AllElement_extract((element_index_extract + 1 - m2_extract):element_index_extract, 3:4) = tmp_e_extract(:, [2,1]);
                        AllElement_extract = [AllElement_extract; tmp_e_extract(:, 1:2), zeros(m2_extract, 2)];
                        % AllElementLabel_extract = [AllElementLabel_extract; ones(size(tmp_e_extract, 1), 1) * NodeLayer_extract{i, 5}(row)];
                    end

                    [element_index_extract, ~] = size(AllElement_extract);
                    AllElement_extract((element_index_extract + 1 - m2_extract):element_index_extract, 3:4) = tmp_end_e_extract(:,[2,1]);
    ...
    AllVelocity_extract = AllVelocity(PointMapping_extract, :);
    AllPointLabel_extract = AllLabel(PointMapping_extract, :);

    % *write the extracted mesh vtk file
    % WriteVTK(hex_extract_output, AllPoint_extract, AllElement_extract, [], AllElementLabel_extract);
    WriteVTKQuad(quad_extract_output, AllPoint_extract, AllElement_extract, AllPointLabel_extract, [], AllVelocity_extract);
