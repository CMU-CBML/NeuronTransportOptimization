function [XYZ_smooth, D_smooth, tangent_vec, n_seg] = BsplineSmoothEqualSegV2(XYZ, D, mode, n_sample, len_seg)
    ...
    if (mode == 1)% branch -> branch 
        if(alphaD * (D(1) + D(end)) < l_spline(end))
%             if(alphaD * (D(1) + D(end))< 0.8 * l_spline(end))
%             seg_desire = linspace(D(1), (l_spline(end) - D(end)), n_seg + 1);
            seg_desire = linspace(D(1) * alphaD, (l_spline(end) - D(end) * alphaD), ceil(( l_spline(end)-D(1)* alphaD-D(end)* alphaD)/len_seg) + 1);
            seg_desire = [0, seg_desire, l_spline(end)];
        end
%         end
    elseif mode == 2% branch -> terminal
        if(alphaD * D(1) < 0.8* l_spline(end))
%             seg_desire = linspace(D(1), l_spline(end), n_seg + 1);
            seg_desire = linspace(D(1)* alphaD, l_spline(end), ceil(( l_spline(end) - D(1)* alphaD)/len_seg) + 1);
            seg_desire = [0, seg_desire]
        end
    elseif mode == 3% root -> branch
        if(alphaD * D(end) < 0.8 *l_spline(end))
%             seg_desire = linspace(0, (l_spline(end) - D(end)), n_seg + 1);
            seg_desire = linspace(0, (l_spline(end) - D(end)* alphaD), ceil(( l_spline(end)-D(end)* alphaD)/len_seg) + 1);
